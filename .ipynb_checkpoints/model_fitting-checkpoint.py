...
def fit_proposed_model(sim_data, jax_prng_key, use_orthogonalization=False):
    """
    ...
    if config.USE_ORTHOGONALIZATION and sim_data["num_interventions_true_K"] > 0:
        ...
        try:
            ...
            bm_pinv = np.linalg.pinv(Bm)
            Z_for_sampler = Z_orig - Bm @ (bm_pinv @ Z_orig)
        except np.linalg.LinAlgError:
            print("       Warning: Orthogonalization failed. Using original Z matrix.")
            Z_for_sampler = Z_orig
