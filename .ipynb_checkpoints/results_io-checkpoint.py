...
def save_raw_posterior_samples(scenario_id, mc_run_idx, posterior_samples, output_dir, model_name="sCFR"):
    """
    Saves the full MCMC posterior samples dictionary to a compressed .npz file.
    The filename now includes the model name to distinguish sCFR and sCFR-O.
    """
    if not posterior_samples: return
    filename = f"scen_{scenario_id}_run_{mc_run_idx+1}_posterior_samples_{model_name}.npz"
    filepath = os.path.join(output_dir, filename)
    os.makedirs(output_dir, exist_ok=True)
    np.savez(filepath, **posterior_samples)

def load_raw_posterior_samples(scenario_id, mc_run_idx, input_dir, model_name="sCFR"):
    """
    Loads raw MCMC posterior samples from a compressed .npz file.
    The filename now includes the model name.
    """
    filename = f"scen_{scenario_id}_run_{mc_run_idx+1}_posterior_samples_{model_name}.npz"
...
def save_posterior_summary_for_run(scenario_id, mc_run_idx, posterior_samples, output_dir, model_name="sCFR"):
    """
    ...
    for param_name in ["alpha", "gamma", "lambda", "phi"]:
    ...
    filename = f"scen_{scenario_id}_run_{mc_run_idx+1}_posterior_summary_{model_name}.npz"
    filepath = os.path.join(output_dir, filename)
    os.makedirs(output_dir, exist_ok=True)
...
def load_posterior_summary_for_run(scenario_id, mc_run_idx, input_dir, model_name="sCFR"):
    """
...
def save_benchmark_results(scenario_id, mc_run_idx, benchmark_results, output_dir):
    """Saves the full time-series results from all benchmark models."""
...
def load_benchmark_results(scenario_id, mc_run_idx, input_dir):
    """Loads the full time-series results for all benchmark models."""
    filename = f"scen_{scenario_id}_run_{mc_run_idx+1}_benchmark_results.npz"
    filepath = os.path.join(input_dir, filename)
    if os.path.exists(filepath):
        try:
