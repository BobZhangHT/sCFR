...
def generate_deterministic_baseline_case_counts(T_sim, T_analysis, function_type, params, min_floor_cases):
    """Generates a deterministic baseline for case counts."""
...
def generate_delay_distribution(T_delay_max, mean_delay, shape_delay):
    """Generates the onset-to-death probability mass function (f_s)."""
...
def construct_Q_matrix(c_t, f_s, T_sim):
    """Constructs the convolution matrix Q."""
    N = T_sim
...
def generate_intervention_input_matrix(T_array_unscaled_sim, intervention_times_true_abs, num_interventions_K):
    """
    Generates the Z_input matrix for step interventions.
    Z[t,k] = I(t >= t_k)
    """
    N_sim = len(T_array_unscaled_sim)
    if num_interventions_K == 0: return np.empty((N_sim, 0))
    
    # Scale to [0, 1] for consistent timing
    denom = max(N_sim - 1, 1)
...
def generate_intervention_hinge_matrix(Z_step):
    """
    Generates hinge basis matrix from step indicators:
    h(t) = (t - t_k)_+, normalized by (T-1).
    """
    if Z_step.size == 0:
        return Z_step
    T = Z_step.shape[0]
...
def generate_random_effects_eta(T_sim, sigma_eta_true, rng_numpy):
    """Generates i.i.d. Gaussian random effects."""
...
def generate_baseline_cfr_zeta(T_sim, T_analysis, cfr_type_code, cfr_params):
    ...
    if cfr_type_code == "C1": # Constant
        zeta_0_t = np.full(T_sim, logit(cfr_params["cfr_const"]))
    elif cfr_type_code == "C2": # Linear
...
def simulate_scenario_data(scenario_config_dict, run_seed):
    ...
    # Calculate Intervention Effect (Step + Hinge)
    ...
    # Return Dictionary (Includes dummy lambda for compatibility)
    return {
        ...
        "true_lambda_0": np.zeros_like(true_beta_abs_0), # Dummy lambda
